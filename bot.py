...
async def get_user_http_session(uid, purpose="general"):
    ...
    if session is None or session.closed:
        # نظام العزل: سعة الـ Connector لكل مستخدم مستقلة لضمان عدم تأثر السرعة بضغط الآخرين
        connector = aiohttp.TCPConnector(
...
def format_simple_card_result(status, card, gateway, response, bin_info=None, elapsed=0.0, extra_field=None):
    sm = {
        "Charged": (f"{gemj('fire')} <b>{bs('𝗖𝗛𝗔𝗥𝗚𝗘𝗗')}</b> {gemj('fire')}", []),
        "Approved": (f"{gemj('check')} <b>{bs('𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗')}</b> {gemj('check')}", []),
...
💳 <b>{bs('𝗖𝗮𝗿𝗱')}</b>
⤷ <code>{card}</code>
🌐 <b>{bs('𝗚𝗮𝘁𝗲𝘄𝗮𝘆')}</b> ━ <code>{gateway}</code>
📝 <b>{bs('𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲')}</b> ━ <code>{response}</code>
{extra}<b>━━━━━━━━━━━━━━━━━</b>
...
<b>━━━━━━━━━━━━━━━━━</b>
⏱️ <b>{bs('𝗧𝗼𝗼𝗸')}</b> ⏱ <code>{elapsed:.2f}{bs('s')}</code>
𝗗𝗲𝘃 → sonik""", he
...
async def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type, filter_type="both"):
    ...
    async def update_ui():
        nonlocal last_ui
        now = time.time()
        if now - last_ui[0] < 8.0 or is_stopped():
            return
