...
BOT_TOKEN = '8658304242:AAEuae9mndG60E7gKLUPI4c2AhoLnq5mF5c'
...
SP_PER_USER_WORKERS = 75
MSP_PER_USER_WORKERS = 75
SITE_PER_USER_WORKERS = 75
...
PRICE_RANGES = {
    ...
    "9": {"name": "0.01-13 USD", "min": 0.01, "max": 13},
}
...
def format_simple_card_result(status, card, gateway, response, bin_info=None, elapsed=0.0, extra_field=None):
    # Always use star emoji for Charged/Approved hits as requested
    header_emoji = PE if status in ["Charged", "Approved"] else ""
    header_eid = [CE["star"]] if status in ["Charged", "Approved"] else []
    
    status_text = status.upper()
    if status == "Charged": status_text = "𝗖𝗛𝗔𝗥𝗚𝗘𝗗"
    elif status == "Approved": status_text = "𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗"
    elif status == "Declined": status_text = "𝗗𝗘𝗖𝗟𝗜𝗡𝗘𝗗"
    elif status == "Error": status_text = "𝗘𝗥𝗥𝗢𝗥"
    
    h = f"{status_text} {header_emoji}".strip()
    
    bi = bin_info or {"brand": "-", "type": "-", "level": "-", "bank": "-", "country": "-", "flag": "🏳️"}
    
    price_val = "-"
    if extra_field:
        price_val = str(extra_field[1]).replace('$', '')
        price_val = f"${price_val}"
    
    price_line = f"𝗣𝗿𝗶𝗰𝗲 ━ {price_val}\n" if extra_field else ""
...
𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 ━ {response}
{price_line}━━━━━━━━━━━━━━━━━
𝗕𝗜𝗡: {bi.get("brand", "-")} | {bi.get("type", "-")} | {bi.get("level", "-")}
𝗕𝗮𝗻𝗸: {bi.get("bank", "-")}
𝗖𝗼𝘂𝗻𝘁𝗿𝘆: {bi.get("country", "-")} {bi.get("flag", "🏳️")}

𝗧𝗼𝗼𝗸 ⏱ {elapsed:.2f}𝘀""", header_eid
...
async def send_channel_hit(res, uid, username, name):
    try:
        ...
        if sv in ["CHARGED", "APPROVED"]:
            ...
            price_val = str(res.get('Price', '-')).replace('$', '')
            if price_val != '-': price_val = f"${price_val}"
            
            msg = f"""𝗛𝗜𝗧 ➛ {bs(sv)} {PE}
𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ➛ {gw}
𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 ➛ {resp}
𝗣𝗿𝗶𝗰𝗲 ➛ {price_val}
𝗨𝘀𝗲𝗿 ➛ <a href="{prof}">{name}</a> (𝗣𝗿𝗲𝗺𝗶𝘂𝗺)"""
            HIT_BUTTON = [[Button.url("🚀 " + bs("Sonik"), f"https://t.me/{MAIN_BOT_USERNAME}")]]
            await styled_send(HIT_CHANNEL_ID, msg, buttons=HIT_BUTTON, emoji_ids=[CE["star"]])
    except:
        pass
...
📝 <i>3DS_REQUIRED | INSUFFICIENT_FUNDS | CARD_DECLINED | ORDER_PAID | CHARGED | PAYMENT_SUCCESSFUL</i>""", emoji_ids=[CE["fire"], CE["fire"], CE["info"], CE["link"]])
        ...
        kb = [
            ...
            [pbtn(f"💰 {bs('0.01-13 USD')}", f"add_price_range:9:{event.sender_id}")],
        ]
...
async def add_proxy_cmd(event):
    ...
    try:
        ...
        if failed:
            for dead_proxy in failed:
                await remove_proxy_by_url(event.sender_id, dead_proxy["proxy_url"])
            await styled_reply(event, f"🗑️ <b>{bs('Removed')} {len(failed)} {bs('dead proxies')}</b>", emoji_ids=[CE["trash"]])
    except Exception as e:
        await styled_reply(event, f"⚠️ <b>{bs('Error')}:</b> <code>{e}</code>", emoji_ids=[CE["cross"]])
...
async def check_proxies_cmd(event):
    ...
    for i, res in enumerate(results):
        if not (isinstance(res, tuple) and res[0]):
            await remove_proxy_by_url(event.sender_id, proxies[i]["proxy_url"])
            dead_proxies_count += 1
    if dead_proxies_count > 0:
        await styled_reply(event, f"🗑️ <b>{bs('Removed')} {dead_proxies_count} {bs('dead proxies')}</b>", emoji_ids=[CE["trash"]])
