...
class CircuitBreaker:
    def __init__(self, failure_threshold=20, recovery_time=15):
    ...
    async def call(self, func, *args, **kwargs):
        ...
        try:
            ...
            if self.failures >= self.failure_threshold:
                self.state = "OPEN"
            raise e
API_CIRCUIT = CircuitBreaker(failure_threshold=20, recovery_time=15)
class ResultCache:
...
def is_card_expired(card):
    """Check if card is expired based on MM|YY format"""
    try:
        parts = card.split('|')
        if len(parts) >= 3:
...
def classify_response(rj):
    ...
    if price is not None and price != '-':
        price = f"${price}"
    rl = ar.lower()
    # Check if response is rejected first
    for rejected in REJECTED_RESPONSES:
        if rejected in rl:
...
𝗗𝗲𝘃 → sonik"""
            ...
            await styled_send(HIT_CHANNEL_ID, msg, buttons=HIT_BUTTON, emoji_ids=[CE["heart_fire"] if sv == "CHARGED" else CE["check"]])
    except:
        pass
    ...
    try:
        ...
        if status == "Rejected":
            try:
                ...
                pass
            return await styled_reply(event, f"🚫 <b>{bs('REJECTED')}</b> 🚫\n<b>━━━━━━━━━━━━━━━━━</b>\n💳 <b>{bs('Card')}</b>\n⤷ <code>{card}</code>\n📝 <b>{bs('Response')}</b> ━ <code>{result.get('Response', 'Card Expired/Invalid')}</code>\n<b>━━━━━━━━━━━━━━━━━</b>\n💡 <i>{bs('Card expired or response not accepted')}</i>", emoji_ids=[CE["cross"]])
...
async def main():
    ...
    while True:
        try:
            ...
            log_system("BOOT", "✅ Rejected responses: empty submit, no valid payment, cart failed, checkout token errors, HTTP_503, card expired")
