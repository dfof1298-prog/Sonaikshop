...
SITE_ERROR_KEYWORDS = [
    ...
    'cart failed with status 429',
    'cart failed with status 503',
    'session id error', 'session_id_error', 'invalid session id',
]
...
async def verify_site_full(site, proxy_data=None, http_session=None, max_retries=1):
    try:
        ...
        if gateway and any(rj.lower() in gw_lower for rj in REJECTED_GATEWAYS):
            return {
                'site': site,
                'status': 'rejected',
        ...
        if gateway in ['dead', 'unknown', 'error', 'rejected_by_api']:
            return {
                'site': site,
                'status': 'rejected',
                'reason': f'Gateway detection failed or rejected: {gateway}',
                'gateway': gateway,
                'price': None
            }
...
𝗗𝗲𝘃 → sonik"""
            ...
            await styled_send(HIT_CHANNEL_ID, msg, buttons=HIT_BUTTON, emoji_ids=[CE["fire_premium"]])
    except:
        pass
...
async def main():
    ...
    while True:
        try:
            ...
            log_system("SHUTDOWN", "Closing all HTTP sessions...")
            try:
                await SESSION_POOL.close_all_sessions()
                for key, session in _USER_HTTP_SESSIONS.items():
                    if not session.closed:
                        await session.close()
                if _GLOBAL_BIN_SESSION and not _GLOBAL_BIN_SESSION.closed:
                    await _GLOBAL_BIN_SESSION.close()
                if _GLOBAL_PROXY_SESSION and not _GLOBAL_PROXY_SESSION.closed:
                    await _GLOBAL_PROXY_SESSION.close()
            except:
                pass
            log_system("SHUTDOWN", "All HTTP sessions closed.")
