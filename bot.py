...
def format_simple_card_result(status, card, gateway, response, bin_info=None, elapsed=0.0, extra_field=None):
    sm = {
        "Charged": (f"𝗖𝗛𝗔𝗥𝗚𝗘𝗗 {gemj('star')}", [CE["star"]]),
        "Approved": (f"𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗 {gemj('check')}", [CE["check"]]),
        "Declined": (f"𝗗𝗘𝗖𝗟𝗜𝗡𝗘𝗗 {gemj('cross')}", [CE["cross"]]),
        "Error": (f"𝗘𝗥𝗥𝗢𝗥 {gemj('warn')}", [CE["warn"]])
    }
    h, he = sm.get(status, sm["Declined"])
    bi = bin_info or {"brand": "-", "type": "-", "level": "-", "bank": "-", "country": "-", "flag": "🏳️"}
    price_line = f"𝗣𝗿𝗶𝗰𝗲 ━ {extra_field[1]}\n" if extra_field else ""
    return f"""{h}
━━━━━━━━━━━━━━━━━
⊀ 𝗖𝗮𝗿𝗱
⤷ <code>{card}</code>
...
async def send_channel_hit(res, uid, username, name):
    try:
        ...
        if sv in ["CHARGED", "APPROVED"]:
            ...
            msg = f"""{gemj('fire_premium')} <b>{bs('HIT')}</b> ➛ <b>{bs(sv)}</b> {gemj('star')}
<b>{bs('Gateway')}</b> ➛ {gw}
<b>{bs('Response')}</b> ➛ {resp}
<b>{bs('Price')}</b> ➛ {res.get('Price', '-')}
<b>{bs('User')}</b> ➛ <a href="{prof}">{name}</a> {gemj('diamond')}"""
